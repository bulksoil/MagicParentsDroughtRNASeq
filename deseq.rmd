...
```{r}
library(DESeq2)
dds <- DESeqDataSetFromMatrix(countData = ctTable, colData = ctMeta, design = ~ Trt + Genotype + Trt:Genotype)

keepers <- rowSums(counts(dds)) >= 30
dds <- dds[keepers,]
dds_gxe <- DESeq(dds, test = "LRT", reduced = ~Trt + Genotype)
dds_g <- DESeq(dds, test = "LRT", reduced = ~Trt + Trt:Genotype)
dds_e <- DESeq(dds, test = "LRT", reduced = ~Genotype + Trt:Genotype)
```

```{r}
gxe_res <- results(dds, pAdjustMethod = "bon")
gxe_res %>% 
  as.data.frame() %>% 
